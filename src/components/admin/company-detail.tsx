        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border-t border-slate-100 pt-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Participant Access</p><p className="mt-1 font-semibold text-slate-950">{company.participantAccessPath ? "Available" : "Not configured"}</p></div><Badge variant="outline" className={company.participantAccessPath ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "bg-slate-50 text-slate-600"}>{company.participantAccessPath ? "Active" : "Unavailable"}</Badge></div>{company.participantAccessPath ? <div className="mt-4 flex flex-wrap gap-2"><Button asChild size="sm" className="gap-2 bg-slate-950"><Link href={company.participantAccessPath} target="_blank" rel="noreferrer">Open Participant Link <ExternalLink className="h-4 w-4" aria-hidden="true" /></Link></Button><CopyLinkButton path={company.participantAccessPath} label="Copy Participant Link" /></div> : <p className="mt-3 text-sm text-slate-500">No participant assessment link is configured.</p>}</div>
            </CardContent>
          </Card>
          <Card className="border-slate-200 shadow-sm">
