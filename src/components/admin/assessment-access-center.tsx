...
import { ArrowLeft, ArrowRight, Building2, Check, ClipboardCheck, KeyRound, UserRound } from "lucide-react";
...
export function AssessmentAccessCenter({ assessments }: { assessments: AssessmentAccessCatalogItem[] }) {
  ...
  function chooseAssessment(id: string) {
    ...
    setState((current) => ({
      ...
      assessmentId: id,
      accessType:
        current.accessType === "company" && !assessment?.companyAvailable
          ? ""
  ...
  if (!started) {
    return <div className="space-y-7"><section className="overflow-hidden rounded-3xl bg-slate-950 p-6 text-white shadow-xl sm:p-8 lg:p-10"><div className="max-w-3xl"><Badge className="border-blue-400/30 bg-blue-500/15 text-blue-100 hover:bg-blue-500/15">Version 1.0 · UI readiness</Badge><h1 className="mt-5 text-3xl font-black tracking-tight sm:text-4xl">Assessment Access Center</h1><p className="mt-3 max-w-2xl text-base leading-7 text-slate-300">Prepare a complete assessment-access request through one guided administrative workflow. This milestone validates the request without creating records or links.</p><Button type="button" size="lg" onClick={() => setStarted(true)} disabled={!assessments.length} className="mt-7 bg-blue-600 font-bold hover:bg-blue-500"><KeyRound className="mr-2 h-5 w-5" />Create Assessment Access</Button></div></section><section className="grid gap-4 sm:grid-cols-3"><Card><CardHeader><CardTitle className="text-base">Dynamic catalog</CardTitle><CardDescription>{assessments.length} current published assessments loaded from the Assessment Definition Engine.</CardDescription></CardHeader></Card><Card><CardHeader><CardTitle className="text-base">Validation only</CardTitle><CardDescription>No company, token, participant link, or manager dashboard will be generated.</CardDescription></CardHeader></Card><Card><CardHeader><CardTitle className="text-base">Execution locked</CardTitle><CardDescription>Issuance becomes available only after Milestone 4 is separately approved.</CardDescription></CardHeader></Card></section></div>;
  }

  return <div className="mx-auto max-w-5xl space-y-6"><header><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-700">Assessment Access Center</p><h1 className="mt-2 text-3xl font-black tracking-tight">Create Assessment Access</h1><p className="mt-2 text-sm leading-6 text-slate-600">Complete all five steps. Nothing will be saved or issued.</p></header>
