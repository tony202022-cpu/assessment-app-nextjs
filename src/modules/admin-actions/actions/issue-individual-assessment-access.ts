...
export const issueIndividualAssessmentAccessAction: AdminActionDefinition<IssueIndividualAssessmentAccessInput,IssueIndividualAssessmentAccessOutput> = {
  ...
  async dryRun(input){return createDryRunPreview({currentState:{access:"Not issued"},expectedResult:{participant:`${input.participantName} · ${input.participantEmail}`,funding:input.fundingType,visibility:input.reportVisibility,language:input.languageMode},affectedRecords:[{type:"access_token",id:"new",label:"Single-use participant link"},{type:"assessment_issuance_policy",id:"new",label:input.reason}],warnings:["The link is a bearer credential and must be shared securely.","No company credit is consumed."]});},
