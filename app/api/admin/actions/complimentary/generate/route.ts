...
const ACTION_ID = "complimentary.generate";
...
function status(code: string) {
  ...
  if (["INPUT_INVALID", "DRY_RUN_FAILED", "CONFIRMATION_INVALID", "CONFIRMATION_REQUIRED", "ASSESSMENT_NOT_ELIGIBLE", "INVALID_EXPIRY"].includes(code)) return 400;
  return 500;
}
...
export async function POST(request: NextRequest) {
  ...
  try { trustedOrigin = getDeveloperTestBaseUrl(); } catch {
    return NextResponse.json(actionFailure({ code: "ACTION_FAILED", message: "The application URL is not configured.", requestId, actionId: ACTION_ID, failedAt: now.toISOString() }), { status: 500 });
  }
  ...
  const rate = consumeRateLimit(`admin-action:${ACTION_ID}:${ipAddress}`, 20, 60 * 60 * 1000);
  if (!rate.allowed) {
    return NextResponse.json(actionFailure({ code: "ACTION_CONFLICT", message: "Too many generation attempts. Try again later.", requestId, actionId: ACTION_ID, failedAt: now.toISOString() }), { status: 429, headers: { "Retry-After": String(rate.retryAfter) } });
  }
  let body: Record<string, unknown> = {};
  try { const parsed = await request.json(); if (parsed && typeof parsed === "object") body = parsed as Record<string, unknown>; } catch {}
  const input = { assessmentId: String(body.assessmentId || "").trim(), expiresAt: String(body.expiresAt || "").trim(), reason: String(body.reason || "").trim() };
  ...
  const context = {
    ...
    resource: { type: "assessment", id: input.assessmentId },
    now,
    ipAddress,
  ...
  return NextResponse.json(result, { status: result.ok ? 200 : status(result.error.code), headers: { "Cache-Control": "no-store" } });
}
