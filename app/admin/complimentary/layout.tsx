...
export default function ComplimentaryAdminLayout({ children }: { children: React.ReactNode }) {
  const session = cookies().get(OFFLINE_ADMIN_COOKIE)?.value;
  if (!isValidAdminSession(session)) redirect(offlineAdminLoginUrl(headers().get(CONTROL_CENTER_RETURN_HEADER), "/admin/complimentary"));
